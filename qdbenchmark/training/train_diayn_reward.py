...
@hydra.main(config_path="config/diayn_reward", config_name="pointmaze")
def train(config: ExperimentConfig) -> None:
    ...
    diayn_smerl = DIAYNSMERL(
        config=diayn_smerl_config,
    ...
    play_step = partial(
        diayn_smerl.play_step_fn, env=env, deterministic=False, skills=skills
    )

    update = diayn_smerl.update
    ...
    while iteration < num_loops:
        ...
        if current_step % config.logging.save_checkpoints_period == 0:
            ...
            saveable_discriminator_params, _ = ravel_pytree(
            ...
            jnp.save(
                os.path.join(
                    _policy_dir,
                    f"discriminator-{current_step}.npy",
                ),
                saveable_discriminator_params,
            )
