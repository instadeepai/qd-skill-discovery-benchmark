...
@hydra.main(config_path="config/map_elites", config_name="pointmaze")
def train(config: ExperimentConfig) -> None:
    ...
    if config.single_init_state:
        keys = jnp.repeat(
            jnp.expand_dims(subkey, axis=0), repeats=config.batch_size, axis=0
        )
    else:
        keys = jax.random.split(subkey, num=config.batch_size)
    ...
    @jax.jit
    def update_scan_fn(
        carry: Tuple[MapElitesRepertoire, RNGKey], unused: Any
    ) -> Tuple[Tuple[MapElitesRepertoire, RNGKey], Any]:
